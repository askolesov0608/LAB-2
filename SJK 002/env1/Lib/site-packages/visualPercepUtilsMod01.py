...
def showInGrid(imgs, m=None, n=None, title="", subtitles=None, aois=None):
    N = len(imgs)
    ...
    for i in range(1, N + 1):
        ...
        img = imgs[i - 1]
        aoi = aois[i - 1] if aois and i <= len(aois) else None

        # Если для изображения указана область интереса, обрезаем его
        if aoi is not None:
