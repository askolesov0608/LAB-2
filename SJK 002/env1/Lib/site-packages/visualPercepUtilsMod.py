...
def showInGrid(imgs, titles=None, subtitles=None, m=None, n=None):
    N = len(imgs)
    m, n = computeRowsCols(N, m, n) if m is None or n is None else (m, n)
    ...
    for i in range(N):
        ...
        if subtitles and i < len(subtitles):
            axs[i].set_xlabel(subtitles[i], fontsize=10)  # Используем xlabel для подзаголовков
        axs[i].axis('off')
